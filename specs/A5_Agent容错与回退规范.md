...
生成文件 Fi
  ...
  └─ 失败 → 进入文件级容错
       ...
       │    → 重试间隔：1s → 2s（固定基数递增，base=1s，factor=2）
       │    → 重试仍失败 → 纳入Agent级L1重试计数
       │
       ├─ L2类：静态检查失败/JSON格式错误
       │    → 文件级修复，MAX_FILE_FIX=2
       │    → 修复间隔：0s（立即修复，携带错误上下文）
...
| 重试间隔 | L1: 1s→3s→5s | L1: 1s→2s（更短，快速恢复单文件） |
| 修复间隔 | L2: 携带错误上下文 | L2: 立即修复（0s，携带文件级错误） |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v1.1 | 2026-04-30 | A+D系列交叉审查修复：文件级重试添加间隔策略L1:1s→2s/L2:立即（#A-2） |
